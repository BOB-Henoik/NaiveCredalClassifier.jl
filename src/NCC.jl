...
function MMI.fit(m::NCClassifier, verbosity::Int, X, y)
    ...
    names_x = Tables.columnnames(X)
    decode_x = tuple(collect(MMI.decoder(Tables.getcolumn(X,n)[1]) for n in names_x)...)
    y_l = MMI.int(y)
    x_l = reduce(hcat, MMI.int.(Tables.getcolumn(X,n)) for n in names_x)

    n_y = length(levels(y))
    n_x = [length(levels(decode_x[x])) for x in range(1,size(x_l,2))]

    cond_count = [zeros(Int64, (n_x[x], n_y)) for x in range(1,size(x_l,2))]
    y_count = zeros(Int64, n_y)
    for row in range(1,size(x_l,1))
        y_count[y_l[row]] += 1
        for col in range(1,size(x_l,2))
            cond_count[col][x_l[row,col],y_l[row]] += 1
        end
    end

    cond_prob = [zeros(Float64, (n_x[x], n_y, 2)) for x in range(1,size(x_l,2))]
    y_prob = zeros(Float64, (n_y,2))

    for col in range(1,size(x_l,2)), val in range(1, n_x[col]), y_cond in range(1,n_y)
...
function MMI.predict(m::NCClassifier, fitresult, Xnew)
    (_, _, decode_y, _, names_x, _, _) = fitresult
	y_hat = Vector{CategoricalValue}[]
    Xmatrix = reduce(hcat, MMI.int.(Tables.getcolumn(Xnew,n)) for n in Tables.columnnames(Xnew))
	for x in eachrow(Xmatrix)
		push!(y_hat, decode_y.(RCB.predict(m.decisionRule, fitresult, x)))
	end
	return y_hat
end
...
function MMI.fitted_params(m::NCClassifier, fitresult)
    ...
    return (; 
        x_probas=(;zip(Symbol.(names_x), [(; zip(Symbol.(decode_x[x].classes), [Dict("$x_val|$y_val" => [cond_prob[x][x_v, y_v,1], cond_prob[x][x_v,y_v,2]] for (y_v,y_val) in enumerate(decode_y.classes)) for (x_v, x_val) in enumerate(decode_x[x].classes)])...) for x in 1:length(names_x)])...),
